...
Views.account = (function () {
  ...
  /* באפליקציה שבחנויות הקישור שבמייל נפתח בדפדפן של הטלפון, לא בה:
     שם החשבון מאושר (או שהסיסמה מתחלפת), ואז חוזרים לאפליקציה */
  function inApp() { return !!(window.Native && Native.is()); }
        ...
        Cloud.resendConfirm(v.email).then(function () {
          ...
          UI.modal({
            ...
            body: '<div class="state-box" role="status" aria-live="polite">' +
                    ...
                    (inApp()
                      ? 'הקישור נפתח בדפדפן ומאשר שם את החשבון. אחר כך חוזרים לכאן ומתחברים.</p>'
        ...
        Cloud.sendRecovery(v.email).then(function () {
          ...
          UI.modal({
            ...
            body: '<div class="state-box" role="status" aria-live="polite">' +
                    ...
                    '<p>אל <span class="mail">' + UI.esc(v.email) + '</span><br>' +
                    (inApp()
                      ? 'הקישור נפתח בדפדפן, ושם קובעים סיסמה חדשה. אחר כך חוזרים לכאן ומתחברים איתה.</p>'
          ...
          if (isSignup && res && res.confirmed === false) {
            UI.modal({
              ...
              body: '<p class="small">צריך ללחוץ על הקישור שבמייל כדי להפעיל את החשבון.</p>' +
                    '<div class="note"><div class="n-ico">💡</div><div>' +
                    (inApp()
                      ? '<b>הקישור נפתח בדפדפן</b> ומאשר שם את החשבון. ' +
                        'אחרי האישור חוזרים לכאן ולוחצים "כבר אישרתי — התחברות".'
