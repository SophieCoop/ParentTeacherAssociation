/* ============================================================
   אשף ההקמה — מסך פתיחה ושניים או שלושה שלבים
   ------------------------------------------------------------
   שלב החשבון הוא חובה: בלי מייל וסיסמה אין כניסה לאפליקציה.
   על שאר השלבים אפשר לדלג.
   ------------------------------------------------------------
   כתובת, איש קשר ותאריכי השנה אינם חלק מההקמה — הם נערכים בהגדרות.
   ------------------------------------------------------------
   החשבון הוא השלב הראשון וגם תנאי להמשך: מרגע שהוא נפתח, כל מה
   שנכנס בשלבים הבאים נשמר בענן תוך כדי ההקמה, ואין שלב שמירה נפרד
   בסוף. עד שהוא נפתח — אין דרך לעקוף את האשף ולהיכנס לאפליקציה.
   ============================================================ */
...
Views.onboarding = (function () {
  ...
  /* החשבון הוא תנאי כניסה. כל עוד לא הוזנו מייל וסיסמה — לא במסך
     הפתיחה ולא בשלבי האשף — אין קיצור דרך שמדלג עליו ונכנס פנימה.
     כשאין ענן מוגדר כלל אין על מה להתחבר, וההקמה נשארת פתוחה כשהייתה. */
  function accountReady() {
    if (!(window.Cloud && Cloud.enabled())) return true;
    if (Cloud.signedIn() || cloud.mode === 'sent' || cloud.mode === 'done') return true;
    /* נרשמו וממתינים לאישור המייל. הסימון שורד רענון, ולכן מי שסגר את
       החלון באמצע אינו נתקע באשף בלי מוצא — המייל והסיסמה כבר הוזנו. */
    return !!(Cloud.pendingSignup && Cloud.pendingSignup());
  }
  ...
  return {
    ...
    actions: {
      ...
      'wiz-exit': function () {
        if (!accountReady()) return;    // בלי חשבון אין כניסה לאפליקציה
